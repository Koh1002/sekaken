...
function MiniChart({ data, dataKey }: { data: DailyActivity[]; dataKey: "studySeconds" | "answers" }) {
  ...
  return (
    <div className="flex items-end gap-[3px] h-16">
      {values.map((v, i) => {
        const height = Math.max((v / max) * 100, v > 0 ? 8 : 2);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card p-3 text-center">
          ...
          <p className="text-[10px] text-[var(--muted)] mt-1">今日の学習</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-2xl font-bold" style={{ color: "var(--primary)" }}>
            {todayAnswers}
          </p>
          <p className="text-[10px] text-[var(--muted)] mt-1">今日の回答数</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-2xl font-bold" style={{ color: "var(--primary)" }}>
            {stats.totalDays}
          </p>
          <p className="text-[10px] text-[var(--muted)] mt-1">学習日数</p>
        </div>
      </div>

      {/* Memory status bar (Monoxer-style) */}
      <div className="card p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-bold">記憶度</h3>
          <span className="text-xs text-[var(--muted)]">{studyCount.studied}件学習済み</span>
        </div>
        <div className="flex gap-1 h-3 rounded-full overflow-hidden" style={{ background: "var(--unknown)" }}>
        ...
        <div className="flex gap-4 mt-2 text-[10px] text-[var(--muted)]">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full inline-block" style={{ background: "var(--memorized)" }} />
            記憶済 {studyCount.learned}
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full inline-block" style={{ background: "var(--in-progress)" }} />
            記憶中 {studyCount.studied - studyCount.learned - studyCount.weak}
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full inline-block" style={{ background: "var(--weak)" }} />
            弱点 {studyCount.weak}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="card p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold">学習時間</h3>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold">回答数</h3>
            <span className="text-xs text-[var(--muted)]">過去14日間</span>
          </div>
